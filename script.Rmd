...
[Video: (en) Protesilaos Stavrou - BASH Parameter Expansion [12 min]](http://www.youtube.com/watch?v=S4D9KaW3ERw)

```{r, include=knitr::is_html_output(), echo=F}
knitr::asis_output('<a href="http://www.youtube.com/watch?v=S4D9KaW3ERw"
                   title="(en) Protesilaos Stavrou - BASH Parameter Expansion [12 min]"
                   ><img src="http://img.youtube.com/vi/S4D9KaW3ERw/0.jpg"></a>')
```




[Video: (en) Pedagogy - Variables In Shell Scripting | store output of a command to a variable | $ [16 min]](http://www.youtube.com/watch?v=3JYhBUK2of0)

```{r, include=knitr::is_html_output(), echo=F}
knitr::asis_output('<a href="http://www.youtube.com/watch?v=3JYhBUK2of0"
                   title="(en) Pedagogy - Variables In Shell Scripting | store output of a command to a variable | $ [16 min]"
                   ><img src="http://img.youtube.com/vi/3JYhBUK2of0/0.jpg"></a>')
```
...
- **Bash Parameter Expansion** ermöglicht die **Manipulation von Variablenwerten**! ([komplette Online-Dokumentation mit Beispielen](https://www.gnu.org/software/bash/manual/html_node/Shell-Parameter-Expansion.html))
...
- `if` - **Verzweigung** über Ja/Nein Test ala "`if [ TEST ]; then A; else B; fi`", wobei `A` und `B` einzelne Anweisungen oder Anweisungssequenzen ("`;`"-getrennt) sein können
  ...
  - Standardtests (Möglichkeiten für `TEST` in obigem Aufruf)
    ...
    - **Datei-/Verzeichnistests**
      - `-e` / `-d` = Datei / Verzeichnis existiert, z.B. `if [ ! -e /tmp/jo.txt ] || [ -s /tmp/jo.txt ]; then echo "man" > /tmp/jo.txt; fi`
...
```{r, include=knitr::is_html_output(), echo=F}
knitr::asis_output('Download: <a href="script.pdf" title="Download PDF Version">PDF Version</a> dieses Tutorials.')
```
